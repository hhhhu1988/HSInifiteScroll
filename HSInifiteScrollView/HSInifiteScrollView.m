...
@interface HSInifiteScrollView() <UIScrollViewDelegate>
...
@implementation HSInifiteScrollView

/** scrollView中UIImageView的数量 */
static NSInteger HSImageViewCount = 3;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        // UIImageView
        for (NSInteger i = 0; i < HSImageViewCount; i++) {
        ...
        // 滚动方向
        self.scrollDirection = HSInifiteScrollDirectionHorizontal;
        
        // 占位图片
        self.placeholder = [UIImage imageNamed:@"HSInifiteScrollView.bundle/placeholder"];
    }
    return self;
}
...
- (void)layoutSubviews
{
    ...
    if (self.scrollDirection == HSInifiteScrollDirectionHorizontal) {
        self.scrollView.contentSize = CGSizeMake(HSImageViewCount * selfW, 0);
    } else {
        self.scrollView.contentSize = CGSizeMake(0, HSImageViewCount * selfH);
    }
...
- (void)updateContentAndOffset
{
    ...
    for (NSInteger i = 0; i < HSImageViewCount; i++) { // i是用来获取imageView的
        UIImageView *imageView = self.scrollView.subviews[i];
